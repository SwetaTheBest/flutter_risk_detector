...
/// Static lint analyzer for Dart VM and Flutter platforms with `dart:io`.
///
/// This analyzer uses lightweight source scanning heuristics. It is designed as
/// a development aid, not as a replacement for the Dart analyzer.
class LintAnalyzer {
  ...
  static final _unawaitedNameRegex = RegExp(
      r'(fetch|load|save|delete|update|post|get|upload|download)\w*\s*\(');
  ...
  static Future<LintResult> analyzeDirectory(String directory) async {
    ...
    try {
      final dartFiles = dir.listSync(recursive: true).whereType<File>().where(
  ...
  static void _checkMissingConst(
      String line, int lineNum, String file, List<LintIssue> issues) {
    if (_constWidgetRegex.hasMatch(line) &&
        !line.contains('const') &&
        !_isComment(line)) {
  ...
  static void _checkVarUsage(
    ...
    if (_varRegex.hasMatch(line) && !_isComment(line)) {
      issues.add(LintIssue(
        ...
        suggestion:
            'Replace var with the explicit type (e.g. String, int, List<...>)',
        severity: LintSeverity.info,
        offendingCode: line,
      ));
    }
  ...
  static void _checkHardcodedColor(
    ...
    if (_hardcodedColorRegex.hasMatch(line) && !_isComment(line)) {
      issues.add(LintIssue(
        ...
        suggestion:
            'Define colors in a theme or constants file and reference them',
        severity: LintSeverity.warning,
        offendingCode: line,
      ));
    }
    ...
    if (_catchRegex.hasMatch(line) || line.trimLeft().startsWith('catch (')) {
      ...
      if (next == '}') {
        issues.add(LintIssue(
          ...
          suggestion:
              'Log the error or rethrow: debugPrint(e.toString()) or rethrow',
  ...
  static void _checkTodoComment(
    ...
    if (_todoRegex.hasMatch(line)) {
      issues.add(LintIssue(
        ...
        suggestion:
            'Resolve or track this in your issue tracker before release',
        severity: LintSeverity.info,
        offendingCode: line,
      ));
    }
  ...
  static void _checkUnawaitedFuture(
    ...
    if (_unawaitedCallRegex.hasMatch(line) &&
      ...
      issues.add(LintIssue(
        ...
        description:
            'Possible unawaited Future — errors will be silently ignored',
    ...
    if (line.contains('setState(') && !_isComment(line)) {
      ...
      if (context.contains('await') && !context.contains('if (mounted)')) {
        issues.add(LintIssue(
          ...
          description:
              'setState() called after async gap without mounted check',
  ...
  static void _checkDebugMode(
      String line, int lineNum, String file, List<LintIssue> issues) {
    if (_debugModeRegex.hasMatch(line) &&
        line.contains('if') &&
        !_isComment(line)) {
      ...
      if (line.contains('kDebugMode') && line.contains('!')) {
        issues.add(LintIssue(
          ...
          suggestion:
              'Verify this is intentional; prefer kReleaseMode for release-only logic',
          severity: LintSeverity.warning,
          offendingCode: line,
        ));
      }
      ...
      for (final entry in _controllerDeclRegexes.entries) {
        ...
        if (!isDisposed) {
          issues.add(LintIssue(
            ...
            description:
                '$ctrl declared but .dispose() not found — memory leak risk',
            suggestion:
    ...
    for (int i = 0; i < lines.length; i++) {
      if (lines[i].contains('StreamSubscription') && !_isComment(lines[i])) {
        issues.add(LintIssue(
          ...
          description:
              'StreamSubscription declared but cancel() not found — memory leak',
          suggestion:
  ...
  static void _checkTimerLeak(
      String source, String file, List<LintIssue> issues, List<String> lines) {
    if (!source.contains('Timer.periodic') && !source.contains('Timer(')) {
      return;
    }
    if (source.contains('timer.cancel()') || source.contains('.cancel()')) {
      return;
    }
    ...
    for (int i = 0; i < lines.length; i++) {
      if (_timerRegex.hasMatch(lines[i]) && !_isComment(lines[i])) {
        issues.add(LintIssue(
          ...
          description:
              'Timer created but cancel() not found — will fire after dispose',
          suggestion:
              'Store the Timer and call timer.cancel() inside dispose()',
          severity: LintSeverity.error,
          offendingCode: lines[i],
        ));
        break;
